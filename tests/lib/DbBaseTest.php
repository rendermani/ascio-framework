...
final class TestDbBaseClass extends TestCase {
    ...
    private function checkDomainAttributes($domain) {
        ...
        $this->assertEquals($domainAttributes->_id, $registrantAttributes->_parent_id, "The registrant should be linked to the parent."); 
    }
    ...
    public function testIncrementalSerialize() : object {
        ...
        $this->assertNotNull($registrantAttributes->_parent_type,"The Registrant->db()->_parent_type should be set"); 
        $this->assertNotNull($registrantAttributes->_parent_db_type,"The Registrant->db()->_parent_db_type should be set"); 
        $this->assertEquals($domain->db()->_id,$registrantAttributes->_parent_id, "The registrant should be linked to the parent."); 
        //no empty updates
        $this->assertFalse(property_exists($serialized,"DomainType"),"The domainType should be empty because not updated"); 
        $this->assertFalse(property_exists($serialized->Registrant,"Email"),"The Registrant Email should be empty because not updated"); 
        $this->assertFalse (property_exists($serialized,"AdminContact"),"The Registrant Email should be empty because not updated"); 
        //only existing updates
        $this->assertNotFalse(property_exists($serialized,"Comment"),"The Comment should be empty because not updated"); 
        $this->assertFalse(property_exists($serialized->Registrant,"Email"),"The Registrant Email should be empty because not updated");         
        return $serialized;        
    }
    /**
